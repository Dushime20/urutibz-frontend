  ...
  return (
    <div className="bg-white rounded-3xl p-4 sm:p-6 shadow-sm border border-gray-100 dark:bg-slate-900 dark:border-slate-700">
      <div className="flex items-center justify-between mb-4 sm:mb-6">
        <h3 className="text-xl font-bold text-gray-900 dark:text-slate-100 hidden sm:block"><TranslatedText text="My Listings" /></h3>
        <div className="flex gap-2 flex-wrap">
          <button onClick={() => onRequestInspection()} className="bg-emerald-600 text-white px-3 py-2 hover:bg-emerald-700 rounded text-sm"><TranslatedText text="Request Inspection" /></button>
          <button onClick={onAddListing} className="bg-teal-600 text-white px-3 py-2 rounded text-sm"><TranslatedText text="Add New Listing" /></button>
        </div>
      </div>
        ...
        {myListings.map((listing) => (
          ...
          >
            ...
            <div className='flex justify-between items-center'>
              <p><TranslatedText text="status" />: {listing.status || <TranslatedText text="Draft" />}</p>
