...
const DashboardMobileNav: React.FC<DashboardMobileNavProps> = ({ items, className }) => {
  ...
  // Don't render on large screens or if no items
  if (!items.length || isLargeScreen) return null;

  return (
    <nav
      aria-label="Mobile dashboard navigation"
      className={`fixed bottom-0 inset-x-0 z-[80] bg-white/95 dark:bg-gray-900/95 border-t border-gray-200 dark:border-gray-800 backdrop-blur safe-area-bottom shadow-[0_-8px_30px_rgba(15,23,42,0.12)] ${className ?? ''}`}
    >
          ...
          return (
            ...
            >
              <span className="relative">
                <Icon className={`w-5 h-5 ${isActive ? 'text-teal-600 dark:text-teal-300' : 'text-black dark:text-gray-300'}`} />
